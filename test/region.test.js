...
describe('Region', function () {
    ...
    describe('#constructor()', function () {
        ...
        it('should throw a TypeError if the strategy is not a function', function () {
            assert.throws(function () {
                let region = new Region(grid, undefined)
            }, TypeError, /strategy must be a function/)
            assert.throws(function () {
                let region = new Region(grid, null)
            }, TypeError, /strategy must be a function/)
            assert.throws(function () {
                let region = new Region(grid, 0)
            }, TypeError, /strategy must be a function/)
            assert.throws(function () {
                let region = new Region(grid, '0')
            }, TypeError, /strategy must be a function/)
            assert.throws(function () {
                let region = new Region(grid, {})
            }, TypeError, /strategy must be a function/)
            assert.throws(function () {
                let region = new Region(grid, [])
            }, TypeError, /strategy must be a function/)
